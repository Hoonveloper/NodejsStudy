...
router.route('/challenge/memo').post(upload.array('photo',1),function(req,res) {
	...
	var photo =req.files;
    console.log('요청 파라미터 : ' + paramName + ', ' + paramDate+', ',paramInfo + ', ', photo[0]);
	...
	if (database) {
        saveData(database,paramName, paramDate, paramInfo,photo,function(err,addedData){
...
var saveData= function(database, name, date, info,photo, callback){
    console.log("save data 호출"+photo.originalname);
    var user = UserModel({"name":name, "date":date, "info":info ,"photo":photo[0].originalname})
